...
const PresentGIF = ({ datas, viewportSize }: any): JSX.Element => {
  ...
  const splitedVal = splitData(datas, noRows).filter(
    (arr: any[]) => arr?.length
  );
  ...
  return (
    <div>
      {/** To change the below big logic we can use column-count : 2 to 4 on above div it should work*/}
      {datas.map((innerVal: any, index: number) => (
        <div key={index}>
          <GIFComp src={innerVal} />
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div id="gif-container" className="d-flex" style={{ cursor: 'hover' }}>
      {Boolean(splitedVal.length) &&
        splitedVal.map((val: any, indexParent: number) => (
      ...
      {Boolean(!splitedVal.length) && <Loader className="m-auto" />}
    </div>
  );
};
